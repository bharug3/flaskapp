        ...
        stage('Deploying to production'){
            ...
            steps {
                ...
                sshagent(['ec2_ssh_production']) 
                    {
                    sh 'chmod +x ec2.py'   
                    sh 'ANSIBLE_HOST_KEY_CHECKING=False ansible -i ec2.py -m ping "tag_Name_${BRANCH_NAME}" -e "ansible_ssh_user=ec2-user"'
                    sh 'pwd'
                    sh 'ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ec2.py ansible/main.yml -e "ansible_ssh_user=ec2-user, targetHost=tag_Name_${BRANCH_NAME}"'
                    }
